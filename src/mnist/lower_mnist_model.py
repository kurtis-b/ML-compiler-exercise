...
def lower_pytorch_to_torch_fx():
    ...
    symbolic_traced: torch.fx.GraphModule = symbolic_trace(MnistNetwork())
...
def lower_pytorch_to_raw_output():
    ...
    m = fx.export_and_import(MnistNetwork(), torch.randn(1, 1, 28, 28), output_type=OutputType.RAW,
...
def lower_pytorch_to_torch_mlir():
    # Export model to torch-mlir
    m = fx.export_and_import(MnistNetwork(), torch.randn(1, 1, 28, 28), output_type=OutputType.TORCH,
                             func_name = "mnist_model")
...
def lower_pytorch_to_linalg_on_tensors():
    # Export model to torch-mlir
    m = fx.export_and_import(MnistNetwork(), torch.randn(1, 1, 28, 28), output_type=OutputType.LINALG_ON_TENSORS, 
                             func_name = "mnist_model")
